...
describe('capture', () => {
  ...
  it('loses progress when the unit walks away', () => {
    ...
    s.units[0].done = false;
    testApply(s, wait(s.units[0].id, [{ x: 0, y: 0 }, { x: 2, y: 0 }]));
...
describe('economy and turn cycle', () => {
  it('pays building income at the start of each turn and heals on owned tiles', () => {
    ...
    testApply(s, { kind: 'endTurn' }); // -> P2
    testApply(s, { kind: 'endTurn' }); // -> P1, turn 2
  ...
  it('recruits from an owned castle, spending gold', () => {
    ...
    const events = testApply(s, { kind: 'recruit', at: { x: 0, y: 0 }, unit: 'knight' });
  ...
  it('rejects unaffordable or misplaced recruitment', () => {
    ...
    expect(() => testApply(s, { kind: 'recruit', at: { x: 0, y: 0 }, unit: 'dragon' })).toThrow(
      IllegalActionError,
    );
    expect(() => testApply(s, { kind: 'recruit', at: { x: 1, y: 0 }, unit: 'soldier' })).toThrow(
      IllegalActionError,
    );
  });
});
...
describe('legality', () => {
  it('refuses to move outside the movement field', () => {
    ...
    expect(() => testApply(s, wait(s.units[0].id, [{ x: 0, y: 0 }, { x: 7, y: 0 }]))).toThrow(
      IllegalActionError,
    );
  });
  ...
  it('refuses to act twice with one unit', () => {
    ...
    testApply(s, wait(s.units[0].id, [{ x: 0, y: 0 }]));
    expect(() => testApply(s, wait(s.units[0].id, [{ x: 0, y: 0 }]))).toThrow(IllegalActionError);
  });

  it('refuses to move an opponent unit', () => {
    const s = testState(makeLevel(['...'], { units: [u(0, 0, 'soldier', 1), u(2, 0, 'soldier', 2)] }));
    expect(() => testApply(s, wait(s.units[1].id, [{ x: 2, y: 0 }]))).toThrow(IllegalActionError);
  });
