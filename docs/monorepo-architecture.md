...
| `@empire/campaign-engine` | 节点状态机、持久名单、战斗桥与存档校验 | 伤害、寻路、射程和战场动画 |
...
默认代码入口是包根。没有 `"./*"` 通配子路径——它会把每个顶层模块变成桶文件之外的第二个名字，连 `__tests__` 都一起公开。

显式例外只有三类：

- 资源子路径（`@empire/game-ui/styles/*`），桶文件不导出样式表
- 题材表现入口 `@empire/story-candidate-01/presentation`，让纯内容消费者不加载 UI 素材图
- 可选重依赖入口 `@empire/game-ui/pixi`，让默认 UI 消费者不加载 GPU 后端

所有包都声明 `"sideEffects": ["**/*.css"]`，否则打包器必须假设 import 桶文件会执行代码，把桶里点到的模块全部保留。

三条架构测试守着这一节。

内容包根入口公开的是可安装的 `ContentPack`、已发布关卡集合和战役定义，不公开组成它们的原始武器、地形、单位数组。体验关卡也只公开取得关卡的函数。入口的精确键集合由架构测试锁住，内部整理不会无意长成公共 API。
