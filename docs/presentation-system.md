...
1. **把这些效果从 CSS 搬进美术**：`filter` 写成 SVG 展示属性，两个后端都看得见，也让纹理可以把阴影一次烘进去。范围明确、可测。上面那一轮把这些规则搬到了题材包自己的样式表里——**归属对了，但它们仍然在 CSS 里**，所以这一条还没做完。
2. **把棋盘相关的 CSS 注入被栅格化的 SVG 文档**（`<style>` 在 SVG 里是生效的）。单一真值仍在 `.css` 文件，但要靠打包器把样式表当文本读进来。
3. **接受 Pixi 后端更平**，投影改用 Pixi 自己的 filter（逐物体投影很贵）、调色用容器级 `ColorMatrixFilter`（一次 GPU pass，便宜）。

推荐第 1 条：它是唯一让「画面长什么样」完整住在美术里的选项，而这正是本页开头那条边界。
