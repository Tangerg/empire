...
| 跨过端口的 | 不跨过的 |
| --- | --- |
| 一层的全部内容（markup），按声明好的深度顺序 | 元素、类名、选择器 |
...
**不缺了。** 两个已知障碍都拆掉了：题材的样子随图走，特效有了自己的原点。剩下的是纯实现：

端口就位之后，GPU 后端还差一件事，而这件事是量出来的：**14 个棋盘上真实存在的 class 的阴影与调色来自样式表，而 markup 自己只声明了 1 处 `filter=`。**

把 markup 栅格化成纹理时，样式表不在场——所以直接做 Pixi 后端会得到一张**没有任何投影、没有地形/地面调色**的画面。三条路：

1. `PixiBoardSurface implements BoardSurface` —— 端口已就位，四个连续属性 + 具名状态 + markup 分层，都是 Pixi 能直接兑现的。
2. **markup 字符串 → 纹理缓存**：同一份 markup 只烘一次。题材美术 459 格 → 19 张纹理（24:1），通用美术每格独立（但那类棋盘本来只有 873 节点）。
3. **display-list 断言**：两个后端必须对「画什么、画在哪、谁盖谁」给出同一个答案。它证明不了纹理烘得对，所以视觉验收仍然要人看一眼。
...
3. **落点节点自带影子而不是靠 filter。** 一张已发布地图上有 459 个直径 2.2px 的落点，每个都挂着 `drop-shadow(0 1px 1px …)`——459 次模糊，只为一个圆点。改成多画一个下移 1px 的深色圆：画面一样，代价从 459 次模糊变成 459 次实心填充。这一条不只在缩放时有用，单位走路的每一帧都少这 459 次。
