...
一个区域必须**有人读**，否则 lint 会警告 `区域 X 声明了但没有任何规则引用`。会读它的路径共五条：某个载荷的引用（目标、条件、效果）、单位的战术指令、部署分配、交战规则、地形覆盖。

这条检查是补上原来只有一半的那道门：引用了未声明的区域一直是**错误**，而声明了没人引用一直没人管。三张已发布的关卡因此各带着一个没接线的区域——`c01-01:north-hill`（剧情让玩家去抢的那座丘，战斗里却没有任何规则提到它）、`c01-05:west-road`（增援落点，坐标在 spawn 里又写了一遍）、以及 `c01-02:middle-bridge`（这个是**假警报**：交战规则的休战区正是它，第一版检查漏了这条引用路径）。前两个已经删掉：一个地方是地形，不是便条。

内置关卡和战役关卡的测试现在断言 lint **一条都不报**（原来只断言没有 error），所以警告是承重的。
...
`三桥河谷` 在这一轮按参考图的布局重做了：24×16，河道南北纵贯并且会拐弯，三处渡口分别在三个不同的行；两岸各有主堡、兵营和四处村庄（其中四处开局中立，值得走过去拿）；道路从每座主堡弯向每处渡口；两处渡口有高地俯瞰；边缘是成线的林带加崖肩，开阔地里散着小片树丛。双方各十二个单位，镜像，包括它的描述一直承诺却从未真正放进关卡的巨龙。

`双子丘陵` 是教学图，恢复成它自己描述里的样子——**只有一座石桥**（之前有两座），两岸各一座主堡、兵营和一处村庄，两处中立村庄，双方各六个单位。`围城` 保持它的城墙与十四回合限时不变，攻方加到十一、守方加到八。`断崖演武场` 没有动：它的描述说明它是一块机制试验台，小是故意的。
