...
## 规则里不许有任何东西由 locale 决定

架构公理写着「这场战斗是确定性的」。而规则里有**七处**用 `left.localeCompare(right)` 打平局。

`localeCompare` 按宿主的 ICU 数据和默认 locale 排序，而 locale 分歧最大的地方正是标点——这些 id 全是标点：`c01.knight-lance`、`formation-line`、`common.heal`。更糟的是，一个不带 ICU 编译的 Node 会退回码点序，也就是**第三个**答案。

这些平局决定的东西不是装饰：支援攻击挑哪把武器、最大伤害搜索留哪把武器，都是这一比；于是**伤害**取决于它。此外还有处理器的执行顺序、单位的初始职业、以及存档要对照的内容清单。两台机器可以把同一场仗打出不同结果，而在任何一台上跑 sweep 都看不出问题。

换成 `byId`（码点序，每个 JS 引擎都同意）之后，replay digest **一位都没变**——本机 locale 下 ICU 和码点序对战役真正碰到的每个平局都给出同样答案，这正是这个风险一直隐形的原因。玩家**读**的东西是另一个问题：一串中文名字需要 collation，那属于表现层，那里 locale 是个真问题。
