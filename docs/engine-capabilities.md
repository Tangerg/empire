...
| 架构依赖测试 | 已实现 | 无环、包边界、组装根唯一性、入口面、行为归属、调用形状、战斗界面归属、通用美术兜底、渲染器端口、题材样式归属、同一答案只写一处、量表读数只算一处、抛出物只翻译一次、样式表无死规则、依赖写在声明里、类型洗白只许一处，共 85 条；断言、朝向、调用形状扫描全部工作区包，错误静默还包含四个应用入口 |
