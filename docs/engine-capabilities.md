...
| 战斗回放 | 已实现 | 不适用 | 不适用 | 关卡 id + 种子 + Action 列表可完整重建战斗。它是引擎属性与证明工具，不是玩家功能：`npm run --silent sweep` 重放全部关卡，`determinism.test.ts` 用 `replayBattle` 守着分歧点 |
...
| 关卡 `BattlePresentation` | 已实现 | 场景画布、分层环境和题材结构；`structure` / `marker` 返回 `null` 表示「无意见」，由通用推导兜底，题材无法把规则跟踪的东西藏起来 |
...
| 架构依赖测试 | 已实现 | 无环、包边界、组装根唯一性、入口面、行为归属、调用形状、战斗界面归属、通用美术兜底、渲染器端口、题材样式归属，共 68 条；断言、朝向、调用形状扫描全部工作区包，错误静默还包含四个应用入口 |
