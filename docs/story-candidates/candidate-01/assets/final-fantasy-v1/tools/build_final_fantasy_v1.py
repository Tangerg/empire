#!/usr/bin/env python3
"""Build the complete C01 cartoon-fantasy runtime library from approved source boards."""
...
MANIFEST = PACK / "manifest-final-fantasy-v1.json"
QA = PACK / "qa-final-fantasy-v1.json"
...
INDEPENDENT_UNITS = [
    "gray-banner-swordsman-source.png", "gray-banner-spearman-source.png",
    "gray-banner-archer-source.png", "gray-banner-cleric-source.png",
    "gray-banner-engineer-source.png", "gray-banner-guard-source.png",
    "imperial-legion-shield-source.png", "silverwood-longbow-guard-source.png",
    "mountain-forge-rune-artificer-source.png", "wasteland-wolf-rider-source.png",
    "cemetery-gravekeeper-source.png", "named-skeleton-guard-source.png",
]

COMBAT_BOARDS = [
    "combat-atlas-ranger-assassin-mage-knight.png",
    "combat-atlas-ballista-cavalry-battlemage-eagle.png",
    "combat-atlas-walker-druid-stag-runeshield.png",
    "combat-atlas-axebreaker-golem-shaman-javelin.png",
    "combat-atlas-heavyknight-spirit-cannon-troll.png",
    "combat-atlas-berserker-templar-inquisitor-ghost.png",
    "combat-atlas-colossus-ivra-wyvern-dragon.png",
]

MISSION_BOARDS = [
    "mission-atlas-farmer-refugees-driver.png",
    "mission-atlas-baker-miner-artisan-bridge.png",
    "mission-atlas-cook-doctor-wounded-wall.png",
    "mission-atlas-scribe-registrar-delegate-messenger.png",
    "mission-atlas-escort-caretaker-militia-quartermaster.png",
    "mission-atlas-controlled-veteran-nursery-witness.png",
]

NARRATIVE_SOURCES = {
    # Character slots. Boards 04 and 06 contain a second useful turnaround in their final cell;
    # topic assignments intentionally use only the authored identities.
    "character-board-01.png": [0, 1, 2, 3],
    "character-board-02.png": [4, 5, 28, 29],
    "character-board-03.png": [30, 31, 32, 33],
    "character-board-04.png": [34, 35, 36, 37],
    "character-board-05.png": [38, 39, 40, 41],
    "character-board-06.png": [42, 43, None, None],
    "prop-board-01.png": [14, 15, 16, 17],
    "prop-board-02.png": [18, 19, 20, 21],
    "prop-board-03.png": [76, 73, 74, 75],
    "prop-board-04.png": [77, 78, 79, 44],
    "scene-board-01.png": [6, 7, 8, 9],
    "scene-board-02.png": [10, 11, 12, 13],
    "scene-board-03.png": [22, 23, 24, 25],
    "scene-board-04.png": [26, 27, 45, 46],
    "scene-board-05.png": [47, 48, 49, 50],
    "scene-board-06.png": [51, 52, 53, None],
    "scene-board-07.png": [54, 55, 56, 57],
    "scene-board-08.png": [58, 59, 60, 61],
    "scene-board-09.png": [62, 63, 64, 65],
    "scene-board-10.png": [66, 67, 68, 69],
    "scene-board-11.png": [70, 71, 72, None],
}
...
def write_asset(image: Image.Image, category: str, topic: dict, meta: dict) -> dict:
    slug = topic["id"].lower().replace("c01-", "").replace("_", "-")
    ...
    if image.mode == "RGBA":
        cleaned = []
        for r, g, b, a in image.get_flattened_data():
            cleaned.append((r, g, b, 0) if (r > 230 and b > 190 and g < 60 and a > 0) else (r, g, b, a))
        image.putdata(cleaned)
    image.save(path, optimize=True)
    payload = {
        "assetId": f"C01-FFV1-{topic['id'][4:]}",
        "topicId": topic["id"], "label": topic["label"], "category": category,
        "png": relative.as_posix(), "width": image.width, "height": image.height,
        "runtimeReady": True, "qualityTier": "final-cartoon-fantasy-v1",
    }
    payload.update(meta)
    return payload
...
def unit_size(index: int, combat: bool) -> tuple[int, int]:
    if not combat:
        return (32, 48)
    if index in {16, 17, 19, 22, 30, 31}: return (64, 48)
    if index in {25, 29, 36, 37, 38, 39}: return (64, 64)
    return (32, 48)
...
def build_units(topics: dict[str, list[dict]], assets: list[dict]) -> None:
    ...
    for name in INDEPENDENT_UNITS:
        im = chroma(MASTERS / "combat-units" / name)
        combat_crops.append([grid_cell(im, 4, 1, c, 0, 8) for c in range(4)])
    for name in COMBAT_BOARDS:
        im = chroma(MASTERS / "combat-atlases" / name)
        for row in range(4):
    ...
    for name in MISSION_BOARDS:
        im = chroma(MASTERS / "mission-atlases" / name)
        for row in range(4):
            mission_crops.append([grid_cell(im, 4, 4, col, row, 5) for col in range(4)])
    assert len(mission_crops) == 24
    for topic, crops in zip(topics["mission-unit"], mission_crops):
        frames = [fit_transparent(c, (32, 48), 1, True) for c in crops]
...
def build_structures(topics: dict[str, list[dict]], assets: list[dict]) -> None:
    specs = [
        ("structures-atlas-01.png", 3, 4), ("structures-atlas-02.png", 3, 4),
        ("structures-atlas-03.png", 3, 4), ("structures-atlas-04-source-4col.png", 4, 4),
        ("structures-atlas-05.png", 3, 4), ("structures-atlas-06.png", 3, 4),
    ]
...
def build_fx(topics: dict[str, list[dict]], assets: list[dict]) -> None:
    ...
    for i in range(1, 7):
        source = MASTERS / "fx" / f"fx-atlas-{i:02d}.png"
        im = chroma(source)
    ...
    for topic, row in zip(topics["fx"], rows):
        ...
        assets.append(write_asset(horizontal(frames), "fx", topic, {
            ...
            "fps": 12, "loop": topic["id"] in {"C01-FX-09", "C01-FX-10", "C01-FX-11", "C01-FX-12"},
...
def build_narrative(topics: dict[str, list[dict]], assets: list[dict]) -> None:
    ...
    for i, topic in enumerate(topics["narrative-static"]):
        cell = assigned[i]
        if i in set(range(0, 6)) | set(range(28, 44)):
            image = fit_opaque(cell, (192, 224)).convert("RGBA")
            kind = "character-card"
        elif i in set(range(14, 22)) | {44, 54, 73, 74, 75, 76, 77, 78, 79}:
...
def contact_sheet(assets: list[dict], filename: str, category_filter: set[str], thumb=(96, 96), cols=8) -> None:
    ...
    for i, asset in enumerate(chosen):
        ...
        draw.text((x + 3, y + thumb[1] + 2), asset["topicId"].replace("C01-", "")[:16], fill=(220, 216, 201))
...
def main() -> None:
    ...
    build_grid_category(topics, assets, "faction-kit", [("faction-kits-12.png", 4, 3)], (128, 128))
    build_terrain(topics, assets)
    build_structures(topics, assets)
    build_grid_category(topics, assets, "battle-prop", [(f"props-atlas-{i:02d}.png", 4, 2) for i in range(1, 5)], (64, 64))
    build_grid_category(topics, assets, "equipment", [
        ("equipment-atlas-01.png", 4, 2), ("equipment-atlas-02.png", 4, 2),
        ("equipment-atlas-03.png", 4, 4), ("equipment-atlas-04.png", 4, 4),
    ], (48, 48))
    build_grid_category(topics, assets, "skill", [(f"skills-atlas-{i:02d}.png", 4, 4) for i in range(1, 4)], (48, 48))
    build_grid_category(topics, assets, "status", [("status-atlas-01.png", 4, 4), ("status-atlas-02.png", 4, 2)], (32, 32))
    build_fx(topics, assets)
    build_grid_category(topics, assets, "hud", [("hud-atlas-01.png", 4, 4)], (32, 32))
    build_narrative(topics, assets)
    order = {k: i for i, k in enumerate(TARGETS)}
    assets.sort(key=lambda a: (order[a["category"]], a["topicId"]))
    manifest = {
        "schemaVersion": "final-fantasy-v1.0", "campaignId": "candidate-01",
        "title": "候选剧本一 · 卡通西幻完整游戏素材包",
        "artDirection": {
            "genre": "original grounded Western fantasy",
            ...
            "prohibitions": ["photorealism", "normal-proportion concept art", "glossy 3D", "named-franchise copying"],
        },
        "runtimeReady": True, "qualityTier": "final-cartoon-fantasy-v1",
