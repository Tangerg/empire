...
这是供真实战棋关卡接入的首批运行时素材，不是 404 题材完整库。BoardView 的“双子丘陵”已实机使用其中 15 项：4 个单位表、8 类地形和 3 个地图建筑。其余 15 项仍是运行时候选，不得从 manifest 整包状态推断为已接入。资产级状态以 `gameIntegration.runtimeReadyAssetIds` 为准；全局预览、覆盖与验收门槛分别见 [Runtime V2 素材画廊](../../../RUNTIME-V2-ASSET-GALLERY.html)、[Runtime V2 覆盖状态](../../../RUNTIME-V2-STATUS.md) 和 [游戏运行时素材契约](../../../GAME-RUNTIME-ASSET-CONTRACT.md)。
...
- BoardView 实机证据：[`../../../runtime-v2-qa/candidate-01-first-level-in-game-final.png`](../../../runtime-v2-qa/candidate-01-first-level-in-game-final.png)
...
第二轮根据 `runtime-v2-qa/candidate-01-first-level-in-game.png` 调整地形：对 ImageGen 材料保留低频结构、压低 1 px 高频噪声，并提升中间调；边缘仍使用同一 32 px 像素连接基准，不改变 56 个 tile 单元及 mask 顺序。
