...
本页定义《布衣定鼎》的机器检查、目检和实机晋级规则。只有进入真实关卡并留下截图证据的素材才能标记为可用。
...
3. 检查单位四帧脚底、兵器裁切、镜像和大型足迹
4. 使用 `3×3` 与随机 `12×8` 地图检查连接地形
5. 检查建筑状态切换、碰撞区、热区和旗帜层
...
当前可先检查 `assets/runtime-v2/previews/runtime-v2-contact-1x.png`、第二批综合预览和第三批战斗单位预览。这些预览不等于实机验收。
