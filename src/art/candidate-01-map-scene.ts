...
import { runtimeAtlasCellMarkup } from './runtime-raster';
import type {
  SceneFrameMarkup,
  SceneLayerMarkup,
  SceneViewport,
...
export function candidate01SceneFrameMarkup(
  ...
  if (levelId !== 'c01-01') return { backdrop: '', foreground: '' };
...
export function candidate01MapSceneryMarkup(levelId: string, map: GameMap): SceneLayerMarkup {
  if (levelId !== 'c01-01') return { ground: '', underUnits: '', overUnits: '' };
  return {
    ground: twinHillsGroundLayer(map),
    underUnits: twinHillsUnderUnits(map),
    overUnits: `<g class="candidate-map-foreground" data-depth="over-units" pointer-events="none">${authoredPlacements(levelId, 'over-units')}</g>`,
  };
}
