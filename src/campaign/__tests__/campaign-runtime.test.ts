...
import { makeLevel, u } from '../../core/__tests__/fixtures';
import { STORY_CAMPAIGN_CONTRACTS } from '../../content/campaigns';
...
describe('generic campaign framework', () => {
  ...
  it('validates all three story adapters against one campaign contract', () => {
    for (const definition of STORY_CAMPAIGN_CONTRACTS) {
      const runtime = new CampaignRuntime(definition);
      expect(runtime.node().type).toBe('story');
      expect(definition.nodes.filter((node) => node.type === 'battle')).toHaveLength(7);
      expect(Object.keys(runtime.state.roster)).toHaveLength(1);
    }
  });
});
