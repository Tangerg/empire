...
describe('editor document aggregate', () => {
  ...
  it('keeps spatial features and units valid when resized', () => {
    const document = EditorDocument.fromLevel(emptyLevel(6, 6));
  ...
  it('restores behavior after history serialization and exports defensive copies', () => {
    const original = EditorDocument.fromLevel(emptyLevel(4, 4));
    const restored = EditorDocument.deserialize(original.serialize());
